...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) FROM pledges INNER JOIN projects ON pledges.project_id = projects.id GROUP BY projects.funding_goal HAVING (SUM(pledges.amount) - projects.funding_goal) >= 0;"
...
def selects_the_category_names_and_pledge_amounts_of_all_pledges_in_the_music_category
  "SELECT projects.category, pledges.amount FROM pledges INNER JOIN projects ON pledges.project_id = projects.id WHERE projects.category = 'music' ORDER BY projects.category ;"
end
